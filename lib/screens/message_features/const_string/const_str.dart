const String trangUsers = 'trang_users';
const String trangChatRooms = 'trang_chat_rooms';

const String chatRoomCollection = 'chat_rooms';
const String usersCollection = 'users';
const String chatCollection = 'chats';

const String emailStr = 'E-mail';
const String activeAtStr = 'active_at';
const String imageStr = 'image';
const String isActiveStr = 'is_active';
const String nameStr = 'name';
const String phoneNumberStr = 'phoneNumber';

const String chatRoomNameStr = 'chat_room_name';
const String usersIdStr = 'users_id';
const String isGroupChatStr = 'group_chat';

const String senderIdStr = 'sender_id';
const String messageStr = 'message';
const String typeStr = 'type';
const String timeStr = 'time';
const String senderImageStr = 'sender_image';

const String senderNameStr = 'sender_name';
const String lastMessageStr = 'last_message';
const String usersImageStr = 'images';
const String usersNameStr = 'names';
const String emailsStr = 'emails';

const String navigateToChatRoomStr = 'chatRoom';
const String loadingDataStr = 'Loading  ...';
const String messageLoadingStr =
    'https://assets7.lottiefiles.com/private_files/lf30_96yLMX.json';
const String messageLoadingStr2 =
    'https://assets7.lottiefiles.com/packages/lf20_Z4BhGL.json';
